...
for _, scheme := range schemes {
    url := fmt.Sprintf(
        "https://screenshotapi.to/api/v1/screenshot?url=https://github.com&colorScheme=%s", scheme)
    ...
    file, _ := os.Create(fmt.Sprintf("github-%s.png", scheme))
...
%w[light dark].each do |scheme|
  uri = URI("https://screenshotapi.to/api/v1/screenshot?url=https://github.com&colorScheme=#{scheme}")
  ...
  File.binwrite("github-#{scheme}.png", response.body)
end
```
</Tab>
<Tab value="PHP">
...
foreach (["light", "dark"] as $scheme) {
    ...
    curl_setopt_array($ch, [
        CURLOPT_URL => "https://screenshotapi.to/api/v1/screenshot?url=https://github.com&colorScheme=$scheme",
    ...
    $image = curl_exec($ch);
    curl_close($ch);
    file_put_contents("github-$scheme.png", $image);
}
```
</Tab>
</Tabs>
