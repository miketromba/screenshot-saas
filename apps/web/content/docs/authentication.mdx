...
1. Navigate to your [dashboard](https://screenshotapi.to/dashboard).
...
<Tab value="Go">
```go
body := strings.NewReader(`{"name": "Production"}`)
req, _ := http.NewRequest("POST", "https://screenshotapi.to/api/v1/api-keys", body)
...
// result["key"] = "sk_live_abc123..." — save this, it won't be shown again
```
</Tab>
<Tab value="Ruby">
```ruby
...
curl_close($ch);
// $response["key"] = "sk_live_abc123..." — save this, it won't be shown again
```
</Tab>
</Tabs>
...
- **Monitor usage** — Check the [usage dashboard](https://screenshotapi.to/dashboard/usage) for unexpected activity.
