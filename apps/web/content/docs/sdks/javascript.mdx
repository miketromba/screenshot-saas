...
class ScreenshotAPI {
  ...
  constructor(apiKey: string, baseUrl = 'https://screenshotapi.dev') {
    this.apiKey = apiKey
...
export async function GET(request: Request) {
  ...
  const response = await fetch(
    `https://screenshotapi.dev/api/v1/screenshot?url=${encodeURIComponent(url)}`,
...
try {
  ...
  if (error instanceof Error) {
    if (error.message.includes('402')) {
      console.error('Out of credits — purchase more at screenshotapi.dev')
