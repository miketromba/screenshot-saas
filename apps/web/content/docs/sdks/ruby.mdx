...
uri = URI("https://screenshotapi.dev/api/v1/screenshot?#{params}")
...
class ScreenshotAPI
  ...
  def initialize(api_key, base_url: "https://screenshotapi.dev")
    @api_key = api_key
    @base_url = base_url
  end
...
begin
  ...
  case e.message
  when /402/
    puts "Out of credits — purchase more at screenshotapi.dev"
  when /403/
