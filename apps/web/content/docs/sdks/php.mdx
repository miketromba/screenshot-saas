...
curl_setopt_array($ch, [
    CURLOPT_URL            => "https://screenshotapi.dev/api/v1/screenshot?{$params}",
...
class ScreenshotAPI
{
    ...
    public function __construct(string $apiKey, string $baseUrl = 'https://screenshotapi.dev')
    {
        $this->apiKey = $apiKey;
...
try {
    ...
    if (str_contains($e->getMessage(), '402')) {
        echo "Out of credits — purchase more at screenshotapi.dev\n";
