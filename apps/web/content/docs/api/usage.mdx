...
<Tab value="cURL">
```bash
curl "https://screenshotapi.dev/api/v1/usage?limit=20&offset=0" \
...
const response = await fetch(
  `https://screenshotapi.dev/api/v1/usage?${params}`,
  { credentials: 'include' }
)
...
response = requests.get(
    "https://screenshotapi.dev/api/v1/usage",
...
<Tab value="cURL">
```bash
curl "https://screenshotapi.dev/api/v1/usage/stats" \
  --cookie "session=your_session_cookie"
```
</Tab>
<Tab value="JavaScript">
```typescript
const response = await fetch('https://screenshotapi.dev/api/v1/usage/stats', {
  credentials: 'include'
})
...
response = requests.get(
    "https://screenshotapi.dev/api/v1/usage/stats",
    cookies={"session": "your_session_cookie"}
)
