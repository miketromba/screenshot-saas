...
Captures a screenshot of the specified URL (or raw HTML via POST) and returns the binary image data directly in the response body. Requires [API key authentication](/docs/authentication). Use `POST` when sending raw HTML via the `html` body parameter.
