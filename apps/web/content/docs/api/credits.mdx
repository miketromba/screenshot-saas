...
<Tab value="JavaScript">
```typescript
const response = await fetch('https://screenshotapi.dev/api/v1/credits/packs')
...
response = requests.get("https://screenshotapi.dev/api/v1/credits/packs")
...
Initiates a credit purchase by creating a Stripe Checkout session. Returns a URL to redirect the user to complete payment. Requires session authentication.
...
3. The user completes payment on Stripe's hosted checkout page.
4. On success, Stripe sends a webhook to `/api/v1/webhooks/stripe`.
...
| Type | Description |
|---|---|
| `purchase` | Credits purchased via Stripe checkout |
...
<Tab value="cURL">
```bash
curl "https://screenshotapi.dev/api/v1/credits/transactions?limit=20&offset=0" \
  --cookie "session=your_session_cookie"
```
</Tab>
<Tab value="JavaScript">
...
const response = await fetch(
  `https://screenshotapi.dev/api/v1/credits/transactions?${params}`,
  { credentials: 'include' }
)
...
response = requests.get(
    "https://screenshotapi.dev/api/v1/credits/transactions",
