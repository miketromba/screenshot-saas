...
ScreenshotAPI supports [webhooks](/docs/api/webhooks) that notify your Zapier workflow when a screenshot completes. This enables **truly asynchronous** workflows where you don't have to wait for the screenshot to render.

### How It Works

1. **Create a Zapier webhook trigger** (Webhooks by Zapier > Catch Hook) and copy the webhook URL
2. **Register the Zapier URL as a webhook endpoint** in the [ScreenshotAPI dashboard](/dashboard/settings) or via the [Webhooks API](/docs/api/webhooks)
