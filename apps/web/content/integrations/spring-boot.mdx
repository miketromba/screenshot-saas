...
ScreenshotAPI works on any platform that runs Spring Boot. No Chromium, no Selenium grid, no special Docker configuration. Whether you deploy on AWS ECS, GCP Cloud Run, or a traditional VM, the integration is a standard HTTP call.
