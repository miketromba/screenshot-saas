...
1. [Create a free account](https://screenshotapi.to) and copy your API key. You get **200 free screenshots per month** immediately.
...
ScreenshotAPI works on every Python hosting platform. No Chromium, no Node.js, no special Docker layers. Whether you deploy on Heroku, Railway, Render, or a plain VPS, the integration is the same `requests.get` call.
